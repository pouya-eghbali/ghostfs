...
namespace ghostfs::http {
  ...
  static void handle_login(const httplib::Request& req, httplib::Response& res) {
    try {
      ...
      json_response(res, 200, fmt::format(R"({{"success":true,"sessionId":"{}"}})", session_id));
  ...
  static void handle_auth_status(const httplib::Request& req, httplib::Response& res) {
    ...
    json_response(res, 200, fmt::format(R"({{"authenticated":true,"user":"{}"}})", session->user));
  }
  ...
  static void handle_list(const httplib::Request& req, httplib::Response& res) {
    ...
    try {
      for (const auto& entry : std::filesystem::directory_iterator(
               resolved, std::filesystem::directory_options::skip_permission_denied)) {
        ...
        entries_json += fmt::format(R"({{"name":"{}","isDir":{},"size":{},"mtime":{}}})",
  ...
  static void handle_stat(const httplib::Request& req, httplib::Response& res) {
    ...
    json_response(
        res, 200,
        fmt::format(
            R"({{"success":true,"stat":{{"name":"{}","isDir":{},"size":{},"mode":{},"mtime":{}}}}})",
            escaped_name, is_dir ? "true" : "false", static_cast<int64_t>(st.st_size),
            static_cast<int>(st.st_mode), static_cast<int64_t>(st.st_mtime)));
  }
  ...
  static void handle_copy(const httplib::Request& req, httplib::Response& res) {
    ...
    if (std::filesystem::is_directory(resolved_src)) {
      std::filesystem::copy(resolved_src, resolved_dest, std::filesystem::copy_options::recursive,
                            ec);
    } else {
  ...
  static void handle_download(const httplib::Request& req, httplib::Response& res) {
    ...
    if (decrypt) {
      ...
      if (!file || file.gcount() < static_cast<std::streamsize>(crypto::HEADER_SIZE)) {
        ...
        ss << file.rdbuf();
        set_cors_headers(res);
        res.set_header("Content-Disposition", fmt::format("attachment; filename=\"{}\"", filename));
      ...
      while (file) {
        ...
        if (!crypto::decrypt_block(block_buf.data(), static_cast<size_t>(bytes_read),
                                   session->encryptionKey->data(), plaintext_buf, &plaintext_len)) {
  ...
  static void handle_upload(const httplib::Request& req, httplib::Response& res) {
    ...
    if (encrypt) {
      ...
      while (remaining > 0) {
        ...
        if (!crypto::encrypt_block(data + offset, block_size, session->encryptionKey->data(),
                                   ciphertext.data())) {
        ...
        out.write(reinterpret_cast<char*>(ciphertext.data()),
                  static_cast<std::streamsize>(ciphertext_size));
  ...
  int start_http_server(const std::string& bind, uint16_t http_port, uint16_t rpc_port,
    ...
    svr->set_exception_handler([](const httplib::Request& /*req*/, httplib::Response& res,
      ...
      try {
        ...
        res.set_content(fmt::format(R"({{"success":false,"error":"{}"}})", e.what()),
                        "application/json");
      } catch (...) {
